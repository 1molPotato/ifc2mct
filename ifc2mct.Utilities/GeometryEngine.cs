...
namespace ifc2mct.Utilities
{
    public class GeometryEngine
    {
        ...
        public static double Volume(IIfcRepresentationItem geom)
        {
            // not implemented
            if (geom is IIfcExtrudedAreaSolid eas)
                return Volume(eas);
            return 3e11;
        }

        public static double Volume(IIfcExtrudedAreaSolid s)
        {
            double depth = s.Depth;
            double profileArea = Area(s.SweptArea);
            return depth * profileArea;
        }

        private static double Area(IIfcProfileDef p)
        {
            // not implemented
            return 1e10;
        }
